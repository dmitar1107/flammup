...
switch ($task) {
	...
	case 'contact':
		...
		mosRedirect( 'index.php?option=com_contact&task=editA&id='. $contact_id );
		break;
...
function editUser( $uid='0', $option='users' ) {
	...
	if ( ( $my->gid < 25 ) && ( $row->gid == 25 ) ) {
		mosRedirect( 'index.php?option=com_users', _NOT_AUTH );
	}
...
function logoutUser( $cid=null, $option, $task ) {
	...
	if ( is_array( $cid ) ) {
		if (count( $cid ) < 1) {
			mosRedirect( 'index.php?option='. $option, 'Please select a user' );
		}
	...
	switch ( $task ) {
		case 'flogout':
			mosRedirect( 'index.php', $database->getErrorMsg() );
			break;
		...
		default:
			mosRedirect( 'index.php?option='. $option, $database->getErrorMsg() );
			break;
	}
}
