...
switch ($_SESSION['type']) {
	...
	case '':
		...
		mosRedirect("index.php?option=login");
	break;
}
...
function seller_menu() {
	...
	HTML_Menu::seller_menu($allmenus);
}
...
function buyer_menu() {
	global $database, $mainframe, $Config_list_limit, $Config_ad_path_site, $_user, $_device_type;
	$query = "SELECT * FROM #__bmenu order by ordering ASC";
	$database->setQuery($query);
	$allmenus = $database->loadObjectList();
	HTML_Menu::buyer_menu($allmenus);
}
